  ...
  return (
    <div className={`grid relative ${wrapperStyles} `} data-tip={dataTip}>
      ...
      <input
        ...
        className={`bg-transparent text-secondary-foreground outline-none border border-border focus:border-teal-500 shadow-teal-100/50 h-9 rounded px-2 w-full placeholder:line-clamp-1 _slowTrans ${className}`}
        {...others}
        required={required}
      />
      {error && <p className={` text-red-500  text-sm mt-1`}>{error}</p>}
